...
> **DW 부대**는 일반 분대(Alpha~Lima)와 달리 NATO 호출부호를 받지 않는 **특수부대**다. 분대원에게 위임해 전투하지 않고, **DW 부대장이 직접 포병 시스템(Dynamic Workflow 엔진)을 가동**한다. 보병이 아니라 **포병**이다. 일반 분대(보병)와 함께 동시 운용되어 제병협동(諸兵協同) 작전을 수행한다.
>
> 🎯 **핵심 설계 목적 — 소대장 컨텍스트 경량화 (계층 분산 설계)**: 소대장이 DW를 직접 돌리면 무거운 DW 작업이 소대장 컨텍스트로 쏟아져 지휘 능력이 떨어진다. *소대장이 포를 쏘면서 어떻게 지휘하겠는가.* 따라서 무거운 DW 포격은 **DW 부대장이 별도 진지(헤드리스 claude 프로세스)에서 전담**하고, 소대장엔 결과만 보고된다. 소대장 컨텍스트 무손상.
>
> 📜 **실증 (2026-05-30) — 2경로 검증**:
> - **경로 1** (부대장이 `Workflow` 도구 직접 보유): ❌ 실패. 팀메이트·서브에이전트엔 `Workflow` 도구 미주입 (`ToolSearch select:Workflow` → `No matching deferred tools found`). Claude Code는 `Workflow` 도구를 최상위 세션에만 준다.
> - **경로 2** (부대장이 `Bash`로 헤드리스 `claude -p` 띄워 DW 가동): ✅ **성공**. `claude -p` 헤드리스 세션 안에서 `ToolSearch select:Workflow`로 도구 로드 → 직접 호출 → 진짜 DW 발동 확인 (Run ID, agent_count, subagent_tokens 검증).
>
> → **결론: DW 부대장은 경로 2(헤드리스 `claude -p`)로 DW를 직접 포격한다.** Claude Code 기본 DW는 최상위에 집중되지만(소대장이 무거워짐), 백호는 부대장이 별도 프로세스로 DW를 전담해 **소대장을 가볍게 유지** — 이것이 백호 계층 분산이 Claude Code 기본 DW보다 우월한 지점이다.
...
5. 결과를 **용병(이종 AI)으로 교차검증** 후 채택 (DW 동종검증 약점 보완)
...
DW 부대는 수십~수백 에이전트라 토큰 소비가 크다. 포병을 함부로 안 쓰듯, 대량 화력이 정말 필요한 작전에만 신중 투입. (백호 모델 차등 철학과 일치)

### 전략적 의의

- **백호의 약점(완전그래프형 대규모 단일 의존)을 DW 부대가 흡수**
- **DW의 약점(동종검증·인간통제 부재·소대장 컨텍스트 집중)을 백호가 보완**: 부대장이 포격 전담 → 소대장 경량 + 용병 이종검증 + 소대장 승인 게이트
- → **백호 = 보병(일반 분대) + 포병(DW 부대) + 특수정찰(용병)의 제병협동(諸兵協同) 지휘 체계**
...
- 모델 단가 차이로 인한 실제 비용 영향은 워크로드 특성에 따라 다르며, 사용 환경에서 별도 측정·벤치마크가 필요하다.

### 이중 구조적 효과 (곱셈적 가능성)

두 구조적 효과는 이론적으로 **곱셈적으로 결합**될 수 있다:
- 통신 경로가 감소한 상태에서 (구조적 효과 1)
- 그 감소된 통신마저 저단가 모델로 처리 (구조적 효과 2)
- → 두 효과의 단순 합산을 초과하는 절감 가능성

> ⚠️ "곱셈적 결합"은 구조적 설계가 만들어내는 이론적 가능성이며, 실제 효과 크기는 워크로드·LLM 모델 단가·실행 패턴에 따라 달라진다. 실측 없이 단정적 비용 절감률로 해석하지 말 것.

이것이 소대 편제 구조 설계의 핵심 가치이다.
