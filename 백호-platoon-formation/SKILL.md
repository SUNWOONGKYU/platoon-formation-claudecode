...
## ★ 실전 회고 반영 — 다분대 코드 작전 교훈

> 다수 분대가 병렬로 코드를 제조·검증·배포한 실전 작전의 회고로 도출된 **구속 규칙 9건.** 각 규칙은 지정 Phase를 보강하며, **코드 생산형 작전에서 특히 강제**된다. (핵심 교훈: 가장 치명적 결함은 분대장 검증이 아니라 소대장의 직접 대조에서 잡혔다.)

**[1군 — 검증이 실제 결함을 잡은 사례. 강제]**

1. **통합 직전 "소대장 기계적 ground-truth 게이트" (Phase 6.5 직전, 신설·필수).**
   분대장 review-evaluate(자가 + 외부 AI)를 모두 통과해도, 소대장이 **직접** `grep`·`git diff`·잔존값 전수 대조(예: 바꾼 상수의 옛값이 운영 경로에 0건인지)를 수행한 뒤에만 통합한다. 근거: 의미 검토는 통과시켜도 기계적 대조만 잡는 결함이 있다 — 입력 스키마의 기본값이 폴백 체인을 무력화해 "수정 완료"를 실효 없는 변경(no-op)으로 만든 사례를 푸시 직전 grep만이 적발했다. **이 게이트 없이 push/배포 금지.**

2. **통합 직전 DW 적대적 코드리뷰 (Phase 2.5 트리거 추가·권장 표준).**
   코드 생산형 소대는 통합 직전 **DW 부대로 적대적 레드팀**(공격면별 병렬 → 발견을 영향범위별로 분류 → 적대적 재검증)을 1회 돌린다. 근거: per-piece review-evaluate가 놓친 **실제 동시성 버그**(재개 락의 이중 실행)를 적대 검토가 적발했다. 독립 적대 각도는 차원별 리뷰가 못 보는 것을 본다.

3. **통합 후 "라이브 런타임 실증 부대" (Phase 6.5 신설·필수).**
   빌드 success·`curl 200`은 동작 증명이 아니다. 전담 분대가 **배포된 산출물을 실제 라이브로 구동**해 실데이터(실제 행·값)로 검증한다. 근거: 라이브 실증이 "빌드됨 ≠ 동작함" 간극을 메웠다.

**[2군 — 조율 마찰 개선]**

4. **파일 소유권 맵 + 공유 파일 flag 프로토콜 (Phase 1.7 보강).**
   작업 분해 시 **분대별 파일 소유권 맵 + 공유 파일 목록을 사전 확정**한다. 규칙: 타 분대/공유 파일을 편집해야 하면 **편집 전 소대장에 flag 필수.** 근거: 무신고 공유 파일 편집이 재작업·원복을 유발했다.

5. **"보고 ≠ ground-truth" — 임계 주장 spot-check (검증 원칙, Phase 6 전반).**
   분대장 보고를 진실로 단정하지 않는다. 소대장/QA는 임계 주장(파일 존재·값·완료 여부)을 **코드·diff·실데이터로 직접 확인**한다. 근거: stale read·오류 보고가 반복됐다(존재하는 파일을 "부재"로, 수정된 값을 "옛값"으로). 직접 확인만이 정정했다.

6. **review-evaluate 합격 기준 — 코드 vs 문서 이원화 (Phase 4 / 검증).**
   **코드 산출물 = 조건부 PASS**(Critical/High = 0 + in-scope 결함 수정 + 범위 외는 백로그 문서화). 로컬 빌드/테스트 불가 환경에선 외부 AI가 코드에 만점을 주지 않으므로 strict 100을 강요하지 않는다. **문서·산출물 = strict 100** 가능. 둘을 구분해 게이팅한다.

**[보조]**

7. **idle·재ack 노이즈 감축 (Phase 4.5 / 9).** idle 분대장에 **비실행성 메시지 금지**(재ack 루프 유발). 완료 태스크 재보고 금지. 소대장은 구체 임무·결정이 있을 때만 메시지한다.

8. **작전 전 비용 추정 게이트 (Phase 1.5 보강).** 분대 수 × 예상 에이전트 × 토큰 ballpark를 지휘관에 제시하고 "이 규모가 병렬 비용을 정당화하나"를 확인한다. 소규모 작업은 풀소대 금지(비용 > 이득).

9. **거짓완료 0 규율 (Phase 6.5 보강).** 미완·degraded·잔여·미검증은 **정직 표기**(완료로 과장 금지). QA가 과장 1건이라도 적발하면 보고 차단·재작성. 백로그는 "통합 후 처리"로 명시한다.

> **한 줄 교훈:** 병렬 분대는 *속도*를, 다층·적대·라이브 검증은 *신뢰*를 준다. 단 "순수 위임"은 맹점이 있어 — 임계 길목(푸시 직전·충돌·통합)에선 **소대장이 직접 ground-truth를 잡아야** 한다.
...
7. **자가수렴 게이트**: 임무 완료 전, 산출물을 임무 요구사항 대비 **스스로 채점**한다(별도 평가 기준 활용). 미달이면 — *동일 방법 반복이 아니라* 원인을 진단해 접근을 바꿔(프롬프트 교정·모델 상향·도구 교체·분해 재설계) 분대원을 재투입한다(최대 N회, 기본 2). N회 수렴 실패 시에만 소대장에 보고.
8. **임무 완료**: TaskUpdate로 status를 completed로 변경 후 결과 보고
...
**실패 처리:** 작업 실패 시 — 결과를 평가해 *실패 원인별로 다른 행동을 동적 선택*(프롬프트 교정·모델 상향·도구 교체·분해 재설계)하며 목표 충족까지 반복(상한 N, 기본 2). *동일 방법 단순 재시도가 아니다.* N회 수렴 실패 시 소대장에게 보고 (소대장이 자율 판단, 지휘관 보고 금지).
