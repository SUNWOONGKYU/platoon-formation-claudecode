...
## Phase 0. 운영 모드 선택 (단일 소대 / 병렬 소대)

스킬 실행 직후, 지휘관에게 **운영 모드**를 먼저 질문한다.
...
운영 모드를 선택해 주십시오:
  ...
  • [단일] 소대 1개 운영 (기본) — 이 세션에서 Agent Teams로 편성
  • [병렬] 소대 여러 개 동시 운영 — tmux로 독립 소대 병렬 전개
```

### 단일 모드 (기본)
→ Phase 1로 바로 진행. 기존 방식대로 Agent Teams 편성.

### 병렬 모드
→ 지휘관이 소대 수와 각 소대별 프로젝트/임무를 지정.
→ tmux 패널을 자동 구성하여 각 패널에서 독립 소대를 편성.
→ 각 소대 내부에서는 기존 Agent Teams 방식(소대장↔분대장 지휘·통제·소통)이 정상 동작.
→ 소대 간 통신은 불가 (각각 독립된 Claude Code 세션).
→ 상세 절차는 **부록: 병렬 소대 운영 (tmux)** 참조.
...
Phase 0에서 지휘관이 **[병렬]**을 선택했을 때 실행하는 절차.
여러 독립 소대를 tmux 패널로 동시에 전개한다.

### 전제 조건

- **복수 프로젝트를 동시에 진행**하여 각각 독립 소대가 필요할 때
- **하나의 대규모 임무를 여러 독립 소대로 분할** 투입할 때 (소대 간 의존성 없음)
- 각 소대는 **독립된 Claude Code 세션**으로 운영되며, 소대 간 통신은 불가
- 소대 내부의 지휘·통제(소대장↔분대장)는 각 세션 안에서 기존 Agent Teams 방식으로 정상 동작
- Windows 환경: Git Bash에서 tmux 사용 + `winpty claude.cmd`으로 Claude Code 실행

### 구성 절차

```
┌─────────────────────────────────────────────────────────────────┐
│  Step 1. 지휘관에게 소대 수 + 프로젝트/임무 확인                     │
│    소대장 → 지휘관 질문:                                           │
│    "몇 개 소대를 투입하시겠습니까?                                   │
│     각 소대가 담당할 프로젝트/임무를 지정해 주십시오."                  │
│                                                                 │
│  Step 2. tmux 세션 생성                                          │
│    $ export TERM=xterm-256color                                  │
│    $ tmux new -d -s multi-platoon                                │
│                                                                 │
│  Step 3. 소대 수에 맞게 패널 분할                                   │
│    2개 소대: 2패널                                                │
│    3개 소대: 3패널                                                │
│    N개 소대: N패널                                                │
│                                                                 │
│    $ tmux split-window -h -t multi-platoon    # 세로 분할          │
│    $ tmux split-window -v -t multi-platoon    # 가로 분할          │
│    $ tmux select-layout -t multi-platoon tiled # 균등 배치         │
│                                                                 │
│  Step 4. 각 패널에서 독립 소대 실행                                  │
│    각 패널에서:                                                    │
│    $ cd {해당 프로젝트 디렉토리}                                     │
│    $ winpty claude.cmd                                           │
│    → 각 세션에서 /platoon-formation 실행하여 독립 소대 편성           │
│    → 각 소대는 Phase 0에서 [단일] 선택하여 정상 편성 진행              │
│                                                                 │
│  Step 5. 각 소대가 독립적으로 작전 수행                               │
│    패널 0: 소대 A — 프로젝트 X 담당                                  │
│    패널 1: 소대 B — 프로젝트 Y 담당                                  │
│    패널 2: 소대 C — 프로젝트 Z 담당 (필요 시)                         │
│                                                                 │
│  ※ 각 소대 내부에서는 기존 Agent Teams 방식으로                       │
│    소대장↔분대장 지휘·통제·소통이 정상 동작한다                        │
└─────────────────────────────────────────────────────────────────┘
```

### 자동 구성 스크립트

소대장이 Bash 도구로 아래 명령을 실행하여 병렬 소대를 자동 구성한다:

```bash
# 2개 소대 병렬 투입 예시
export TERM=xterm-256color
SESSION="multi-platoon"

# 소대 A 패널 (프로젝트 X)
tmux new -d -s $SESSION -c "/c/Users/home/Desktop/project-x"
tmux send-keys -t $SESSION "winpty claude.cmd" Enter

# 소대 B 패널 (프로젝트 Y)
tmux split-window -h -t $SESSION -c "/c/Users/home/Desktop/project-y"
tmux send-keys -t $SESSION "winpty claude.cmd" Enter

# 균등 배치
tmux select-layout -t $SESSION tiled

# 각 패널의 Claude Code에서 /platoon-formation 실행 → [단일] 선택하여 편성
```
...
| 키 | 동작 |
|----|------|
| `Ctrl+B` → 방향키 | 소대(패널) 간 이동 |
| `Ctrl+B` → `z` | 현재 소대 전체화면 토글 (줌) |
| `Ctrl+B` → `d` | tmux에서 빠져나가기 (세션 유지) |
| `tmux attach -t multi-platoon` | 다시 붙기 |

### 원격 모니터링 (Termius)

외출 시 폰/태블릿에서 Termius로 SSH 접속 후:
```
tmux attach -t multi-platoon
```
각 소대의 작전 상황을 실시간으로 확인할 수 있다.
`Ctrl+B` → 방향키로 소대를 이동하며 상태를 점검한다.
`Ctrl+B` → `z`로 특정 소대를 확대하여 상세 확인 가능.

### 주의사항

- tmux는 Git Bash에서만 동작. SSH 기본 셸을 Git Bash로 설정해야 한다.
- Claude Code는 `winpty claude.cmd`으로 실행해야 tmux 내에서 동작한다.
- 같은 파일을 여러 소대가 동시에 수정하면 충돌 위험 → 소대별 프로젝트/작업 영역을 분리한다.
- 패널이 많아지면 가독성 저하 → `Ctrl+B` → `z`로 줌 활용.
- 소대 간 통신은 불가. 소대 간 의존성이 있는 임무는 병렬 모드 부적합.
